...
for folderb in [f for f in os.listdir(patha) if (not "." in f)]:
    ...
    df_VOTAVEL = pd.read_parquet(
        os.path.join(pathb, "VOTAVEL.parquet"), engine="pyarrow"
    )
